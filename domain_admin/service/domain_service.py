...
def _is_importable_cert_domain(domain):
    """
    Only import domains that are currently resolvable and expose a matching
    certificate on 443. CT logs contain historical names that may no longer
    be in service.
    """
    try:
        hosts = cert_socket_v2.get_domain_host_list(domain=domain, port=443)
    except Exception:
        logger.info("skip inactive cert domain, resolve failed: %s", domain)
        return False
    ...
    logger.info("skip inactive cert domain, ssl unavailable: %s", domain)
    return False


def _get_common_subdomain_candidates(root_domain):
    prefixes = [
        'www', 'app', 'api', 'admin', 'portal', 'merchant', 'gateway', 'pay',
        'cashier', 'pos', 'acquiring', 'bank', 'bankapi', 'sandbox', 'uat',
        'dev', 'test', 'pre', 'preapi', 'prod', 'console', 'open', 'docs',
        'developer', 'developers', 'help', 'files', 'static', 'cdn',
        'download', 'm', 'h5', 'web', 'mail', 'smtp', 'imap', 'mx',
    ]
    return [
        {'common_name': '%s.%s' % (prefix, root_domain)}
...
def auto_import_from_domain(root_domain, group_id=0, user_id=0):
    ...
    if not lst:
        logger.info("crt.sh empty or unavailable, fallback to certspotter: %s", root_domain)
        try:
            certspotter_rows = certspotter_api.search(root_domain, include_subdomains=True)
            for row in certspotter_rows:
                for dns_name in row.get('dns_names') or []:
                    lst.append({'common_name': dns_name})
        except Exception:
            logger.error(traceback.format_exc())

    lst.extend(_get_common_subdomain_candidates(root_domain))
    ...
    for domain in lst:
        ...
        if _is_importable_cert_domain(common_name):
